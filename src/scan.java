...
public class scan {
	 ...
	 */
	public static void main(String[] args) throws SQLException, IOException, BiffException, RowsExceededException, WriteException {
		Connection c ;
	...
	public static void  setup(Connection c, Workbook data) throws SQLException	{
		...
		Statement mystatement = c.createStatement();
		 ...
		 rs = mystatement.executeUpdate("CREATE TABLE JOURS_FERIES(JOUR DATE, MEDECIN INTEGER, INTERIEUR BOOLEAN, FOREIGN KEY (NUMERO) REFERENCES MEDECINS(NUMERO),CONSTRAINT ENTRY_DD primary key (JOUR, INTERIEUR))");
		 if (!hasint){
 ...
 public static void filltables(Connection c, Workbook data) throws SQLException {
	...
	for (int i = 1; i < sheet.getRows();i++){
		if(sheet.getCell(i,3).getCellFormat() != null){
			rs = mystatement.executeUpdate("INSERT INTO SERVICES(NOM, INTERIEUR) VALUES(".concat(sheet.getCell(i,3).getContents()).concat(",TRUE)"));
		}
		else{
			rs = mystatement.executeUpdate("INSERT INTO SERVICES(NOM, INTERIEUR) VALUES(".concat(sheet.getCell(i,3).getContents()).concat(",FALSE)"));
		}
	}
	...
	for (int i = 1; i < sheet.getRows();i++) {
		...
		rs = mystatement.executeUpdate("INSERT INTO MEDECINS(NOM,NBSEMESTRES,SERVICE) VALUES(".concat(sheet.getCell(0,i).getContents()).concat(",").concat(sheet.getCell(1,i).getContents()).concat(Integer.toString(nservice)).concat(")"));
	}
	...
	for (int i = 1; i < sheet.getRows();i++){
		...
		rs = mystatement.executeUpdate("INSERT INTO IMPOSIBILITES(DATEDEBUT,DATEFIN,NUMERO) VALUES(".concat(sheet.getCell(0,i).getContents()).concat(",").concat(sheet.getCell(1,i).getContents().concat(Integer.toString(nmedecin)).concat(")")));
	}
	...
	for (int i = 1; i < sheet.getRows();i++){
		...
		int nmedecin = rs2.getInt("NUMERO");
		rs = mystatement.executeUpdate("INSERT INTO JOURS_FERIES(DATE,MEDECIN) VALUES ('".concat(sheet.getCell(0,i).getContents()).concat("',").concat(Integer.toString(nmedecin)));
	}
 }
