...
public class scan {
	...
	public static boolean  setup(Connection c, Workbook data) throws SQLException	{
		...
		Statement mystatement = c.createStatement();
		 ...
		 rs = mystatement.executeUpdate("CREATE TABLE MEDECINS(NUMERO INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 1, INCREMENT BY 1)PRIMARY KEY, NOM VARCHAR(20), DERNIEREGARDE Date, NBGARDES INTEGER, NBLUNDI INTEGER default 0, NBMARDI INTEGER default 0, NBMERCREDI INTEGER default 0, NBJEUDI INTEGER default 0, NBVENDREDI INTEGER default 0, NBSAMEDI INTEGER default 0, NBDIMANCHE INTEGER default 0, SERVICE INTEGER,NBSEMESTRES INTEGER, NBFERIES INTEGER, FOREIGN KEY (SERVICE) REFERENCES SERVICES(NUMERO))");///< crée la table des medecins avec nom, id et nombre de gardes
		 rs = mystatement.executeUpdate("CREATE TABLE IMPOSSIBILITES(DATEDEBUT Date, DATEFIN date, NUMERO INTEGER, FOREIGN KEY (NUMERO) REFERENCES MEDECINS (NUMERO))");///< table des vacances
		 if(hasint){
	...
	bftest = res.gtg;
	 ...
	 if (rs.getInt("NBSEMESTRES") == 0){
		...
		if(interieur){
		 ...
		 bftest = res.gtg;
		 res.gtg = res.gtg && (rs.getInt("NBGARDES") < 5) && (nbdays > repos);
		 if(bftest && !res.gtg){
			 res.error = "plus de 5 gardes pour un semestre 3/4";
		 }

		 if(dowtoinc == "NBJEUDI"){
			 bftest = res.gtg;
			 res.gtg = res.gtg && (rs.getInt(dowtoinc) == 0);
			 if(bftest && !res.gtg){
				 res.error = "semestre 3/4 ayant deja fait un jeudi";
			 }
			 ...
			 res.gtg = res.gtg && (rs.getInt(dowtoinc)==0);
			 if(bftest && !res.gtg){
				 res.error = "semestre 3/4 ayant deja fait un vendredi";
			 }
			 
		 }
		 else if (dowtoinc == "NBDIMANCHE"){
			 bftest = res.gtg;
			 res.gtg = res.gtg && (rs.getInt(dowtoinc) == 0) && (rs.getInt("NBFERIES") == 0);
			 if(bftest && !res.gtg){
				 res.error = "semestre 3/4 ayant deja fait un dimanche ou un jour férié";
			 }
		 }
		 else if(dateferiee(curdat,c)&&dowtoinc!="NBDIMANCHE"){
			 bftest = res.gtg;
			 res.gtg = res.gtg && (rs.getInt("NBDIMANCHE") == 0);
			 if(bftest && !res.gtg){
				 res.error = "semestre 3/4 ayant deja fait un dimanche : pas de férié";
			 }
		 ...
		 bftest = res.gtg;
		 res.gtg = res.gtg && rs.getInt("NBGARDES") < 3 && !dateferiee(curdat,c);
		 if(bftest && !res.gtg){
			 res.error = "plus de 3 gardes pour un 5+ semestre";
		 }
		 ...
		 if((dowtoinc == "NBVENDREDI")||(dowtoinc == "NBSAMEDI")||(dowtoinc=="NBDIMANCHE")){
			 ...
			 res.gtg = false;
			 if(bftest && !res.gtg){
				 res.error = "pas de vendredi,samedi ou dimanche pour un 5+ semestre";
			 }
