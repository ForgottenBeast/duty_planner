...
public class scan {
	...
	public static boolean  setup(Connection c, Workbook data) throws SQLException	{
		...
		boolean hasint = false;
		 ...
		 if(!hasint){
			 System.out.println("pas d'interieur");
		 }
		Statement mystatement = c.createStatement();
		 ...
		 rs = mystatement.executeUpdate("CREATE TABLE JOURS_FERIES(JOUR varchar(20), NUMERO INTEGER, INTERIEUR BOOLEAN, FOREIGN KEY (NUMERO) REFERENCES MEDECINS(NUMERO),CONSTRAINT ENTRY_DD primary key (JOUR, INTERIEUR))");
			 ...
			 rs = mystatement.executeUpdate("CREATE TABLE JOURS_FERIES(JOUR varchar(20) primary key, NUMERO INTEGER, FOREIGN KEY (NUMERO) REFERENCES MEDECINS(NUMERO))");
		 }
		 if (!hasint){
...
while(rs2.next()){
	...
	for (int i = 1; i < sheet.getRows();i++){
		...
		rs = mystatement.executeUpdate("INSERT INTO JOURS_FERIES(JOUR,NUMERO,INTERIEUR) VALUES ('".concat(formatter.format(formatter.parse(sheet.getCell(0,i).getContents()))).concat("',").concat(Integer.toString(nmedecin).concat(",").concat("TRUE").concat(")")));
			...
			int nmedecin = rs2.getInt("NUMERO");
			rs = mystatement.executeUpdate("INSERT INTO JOURS_FERIES(JOUR,NUMERO) VALUES ('".concat(sheet.getCell(0,i).getContents()).concat("',").concat(Integer.toString(nmedecin)).concat(")"));
		}
		}
		}
 }
 ...
 public static void genplanning(Connection c, Workbook data,boolean hasint) throws ParseException, SQLException{
	 ...
	 while(!curdat.after(datefin)){
		 ...
		 dunit garde = new dunit(666, dowtoinc, dowtoinc, curgarde, curgarde, curgarde, curgarde, true);
			...
			garde = selecttoubib(hasint,repos,curg,prevurg,prevint,true,newdowcount,curgarde,c,curdat,true,dowtoinc,formatter);
			curg = garde.curg;
			...
			dorecord(c,garde,true,formatter,hasint);
			}
			else{
				prevurg = 666;
			}
		 garde = selecttoubib(hasint,repos,curg,prevurg,prevint,true,newdowcount,curgarde,c,curdat,false,dowtoinc,formatter);
		 if(garde.medundefined){
				System.out.println("arr outta luck");
				break;
			}
		 dorecord(c,garde,false,formatter,hasint);
		prevurg = garde.curg;
		prevint = curg;
		cal.setTime(curdat);
 ...
 public static dunit selferie(boolean hasint,Connection c,Date curdat, dunit garde,String dowtoinc,boolean interieur,SimpleDateFormat fmt) throws SQLException{
	 Statement ms= c.createStatement();
	 Statement ms2 = c.createStatement();
	 ResultSet rs,rs2;
	 dunit res = garde;
	 if(!interieur){
		 ...
		 if(hasint){
			 ...
			 rs = ms.executeQuery("SELECT M.NUMERO as NUMERO, M.DERNIEREGARDE as DERNIEREGARDE, M.NBGARDES as NBGARDES,M.".concat(dowtoinc).concat(" as "+dowtoinc+",SERVICE FROM MEDECINS AS M JOIN JOURS_FERIES AS JF ON NUMERO = JF.NUMERO WHERE JF.JOUR = '").concat(fmt.format(curdat)).concat("'"));
		 }
			 while(rs.next()){
			 int nbferie = 0;
 ...
 public static boolean isreserved(boolean hasint,boolean interieur,Connection c,Date curdat, SimpleDateFormat fmt) throws SQLException{
	 boolean itis = false;
	 Statement ms = c.createStatement();
	 ResultSet rs2;
	 if(hasint){
	 rs2 = ms.executeQuery("SELECT NUMERO FROM JOURS_FERIES WHERE JOUR = '".concat(fmt.format(curdat))+"' AND INTERIEUR = "+interieur);
	 }
	 else{
		 rs2 = ms.executeQuery("SELECT NUMERO FROM JOURS_FERIES WHERE JOUR = '".concat(fmt.format(curdat).concat("'"))); 
	 }
	 while(rs2.next()){
		 itis = true;
	 }
 ...
 public static dunit selecttoubib(boolean hasint,int repos, int curg,int prevurg, int prevint,boolean medundefined,int newdowcount,int curgarde,Connection c,Date curdat,boolean interieur,String dowtoinc,SimpleDateFormat fmt) throws SQLException, ParseException{
	 Statement ms2 = c.createStatement();
	 Statement ms = c.createStatement();
	 boolean ferie = false;
	 ...
	 if(dateferiee(curdat,c,fmt)){
		 if(isreserved(hasint,interieur,c,curdat,fmt)){
		 res = selferie(hasint,c,curdat,res,dowtoinc,interieur,fmt);
		 if(!res.medundefined){
		 ...
		 while(rs.next()){
					 ...
					 if(isgtg(curg,prevint,prevurg,c,curdat,fmt,rs,dowtoinc,interieur,repos)){
						 ...
						 res.dowtoinc = dowtoinc;
						 System.out.println("now gtg, assigning jour");
						 res.jour = fmt.format(curdat);
...
public static void dorecord(Connection c, dunit garde,boolean interieur,SimpleDateFormat fmt,boolean hasint) throws SQLException, ParseException{
...
public static void writegardes(Connection c, WritableWorkbook output,boolean hasint) throws SQLException, RowsExceededException, WriteException, IOException{
	...
	if(hasint){
		...
		System.out.println("requesting not hasint");
	...
	ms.addCell(l2);
	while(rs.next()){
		System.out.println("writing garde");
		l1 = new Label(0,i,rs.getString("JOUR"));
